...
*/
/*
arriba, abajo, izquierda, derecha o diagona
*/
...
function largestGridProduct(arr) {
  let sumaMaxima = 0;
  let diagonalDerecha = 1;
  let vertical = 1;
  let horizonta = 1;
  let diagonalIzquierda = 1;

  for (let i = 0; i < arr.length; i++) {
    for (let j = 0; j < arr.length; j++) {
      for (let k = 0; k < 4; k++) {
        if (i <= arr.length - 4 && j <= arr.length - 4) {
          diagonalDerecha *= arr[i + k][j + k];
        }
        if (i <= arr.length - 4 && j >= 3) {
          diagonalIzquierda *= arr[i + k][j - k];
        }
        if (i <= arr.length - 4) {
          vertical *= arr[i + k][j];
        }
      ...
      if (diagonalDerecha > sumaMaxima) {
        sumaMaxima = diagonalDerecha;
      }
      if (diagonalIzquierda > sumaMaxima) {
        sumaMaxima = diagonalIzquierda;
      }
      if (vertical > sumaMaxima) {
        sumaMaxima = vertical;
      }
      if (horizonta > sumaMaxima) {
        sumaMaxima = horizonta;
      }
